<template lang="pug">
  ...
  <v-flex xs12="xs12">
    <v-select v-if="['select', 'select2'].includes(field.type)" :items="field.choices" v-model="model" v-bind="field"></v-select>
    <template v-else-if="['radios', 'checkboxes'].indexOf(field.type) &gt; -1">
      ...
      <v-layout row="row" wrap="wrap">
        <v-flex v-bind="{[field.width]: true}" xs12="xs12" v-for="option in field.choices" :key="field.value">
          <component v-model="model" hide-details="hide-details" :is="field.type == 'radios' ? 'v-radio' : 'v-checkbox'" :key="option.value" :value="option.value" :label="option.text"></component>
        </v-flex>
      </v-layout>
    </template>
