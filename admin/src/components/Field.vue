<template>
  <v-flex xs12="xs12">
    <v-select v-if="['select', 'select2'].includes(field.type)" :items="field.options" v-model="model" v-bind="field"></v-select>
    <template v-else-if="['radios', 'checkboxes'].indexOf(field.type) &gt; -1">
      <p>{{$t(field.label)}}</p>
      <v-layout row="row" wrap="wrap">
        <v-flex v-bind="{[field.width]: true}" xs12="xs12" v-for="option in field.options" :key="field.value">
          <component v-model="model" hide-details="hide-details" color="blue" :is="field.type == 'radios' ? 'v-radio' : 'v-checkbox'" :key="option.value" :value="option.value" :label="option.text"></component>
        </v-flex>
      </v-layout>
    </template>
    ...
    <template v-else-if="['switch'].indexOf(field.type) &gt; -1">
      <v-switch :label="$t(field.label)" v-model="model" color="blue"></v-switch>
    </template>
    ...
    <v-text-field v-else="v-else" v-model="model" v-bind="field" :label="$t(field.label)" :placeholder="$t(field.placeholder)" type="text" :multiLine="field.type == 'textarea'"></v-text-field>
  </v-flex>
</template>
...
export default {
  ...
  methods: {
    ...
    initImageLoad(){
      if(this.field.type&&['file', 'image', 'video'].includes(this.field.type)){
